...
As usual, keyboard layout may be wrong with the desired one, so you'll have to change it by typing:
...
Now it's time to prepare partitions. At least a partition ``/`` is needed, but you could also split these partition from ``/var``, ``/home``, ``swap`` partition,... In order to do that I used parted, as explained in Arch's wiki, but ``fdisk`` and ``gdisk`` could also fit that porpouse.

I needed to do a triple boot, because I also have Widnows and Ubuntu installed in the same machine, and I wanted to keep Ubuntu's boot, so once I created ``/`` partition, I mounted both this partition and EFI's one in a folder in ``mnt``:
...
To install classic Gnome Network Manager the below steps are suggested:
...
The above commands make Network Manager start on boot, stop and disable starting on boot of dhcpd in order for NetworkManger to be the handler of that function.
...
-- Use of ``chroot``. I hadn't use that command untill I've come to install Arch, and it can be very handy in server management and repairing of Linux installations through Live CDs and other already installed systems.
...
* It's *hotplug capable*: that means that if a device, like a hard disk, is pluged in the middle or even after the boot, the system will mount as normal, whereas if ``init``is the handler of initialization that doesn't work the same way.
![](https://github.com/dgiim/blog/blob/gh-pages/images/ubuntu_press_s_to_mount.png){: height="275px" width="200px"}

* It's easier to configure, because the old ``tc.sysinit`` has been split up in several files.

* It handles daemons better.

* It's a *cross-distro-project* which developers of lots of distros have been working on. And the ``systemd`` scripts will be distributed all the same for whatever distro we're talking about, whereas in the older times, each distro had it's own ``rc scripts``.
