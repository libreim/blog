...
**Definición 1:**
: Sea $$A \subseteq \mathbb{R}$$ y $$\{f_n\}$$ una sucesión de funciones con $$f_n : A \rightarrow \mathbb{R}$$. Se dice que $$\{f_n\}$$ **converge puntualmente** en $$B \subseteq A$$ si para cada $$x \in B$$ la sucesión $$\{f_n(x)\}$$ converge. Podemos considerar el conjunto $$C$$ de los elementos $$x \in A$$ en los que $$\{f_n(x)\}$$ converge y definir la **función límite**:
...
**Definición 2:**
: Sea $$A \subseteq \mathbb{R}$$ y $$\{f_n\}$$ una sucesión de funciones con $$f_n : A \rightarrow \mathbb{R}$$. Se dice que $$\{f_n\}$$ **converge uniformemente** en $$B \subseteq A$$ si para cada $$\epsilon > 0$$ existe $$n_o \in \mathbb{N}$$ tal que:
: $$\forall n \ge n_o : \left| f_n(x) - f(x) \right| < \epsilon \ \forall x \in B$$
...
**Ejemplo 2:**
: Estudia la convergencia puntual y uniforme de la sucesión de funciones $$\{f_n\}$$ donde $$f_n : \mathbb{R} \rightarrow \mathbb{R}$$ esta definida por:
...
y de haber convergencia uniforme para cualquier sucesión de números reales $$\{x_n\}$$ se tiene que la sucesión $$\{f_n(x_n) - f(x_n)}$$ converge a 0.
...
Sea $$\epsilon > 0$$, utilizaremos la continuidad de $$\phi$$ junto con la continuidad de la función exponencial para obtener el resultado deseado.

Por la continuidad de la exponencial en 0, existe $$\delta_1 > 0$$ tal que si $$\vert u \vert < \delta_1$$ entonces $$\vert e^u -1 \vert < \frac{\epsilon}{\exp\left(\alpha^2\right)}$$. Aplicamos ahora la continuidad de $$\phi$$ en 0 para obtener $$\delta_2 > 0$$ tal que si $$\vert t\vert < \delta_2$$ entonces $$\vert \phi(t) -1\vert < \frac{\delta_1}{\alpha^2}$$. 
...
\left| x^2 \left( \phi \left(\frac{x^2}{n}\right) -1 \right) \right| < \delta_1 
\Rightarrow \left| \exp\left(x^2\left(\phi \left(\frac{x^2}{n}\right) - 1\right)\right) - 1\right| < \frac{\epsilon}{\exp(\alpha^2)}$$

De donde se deduce que 

$$\left| \left(1+\frac{x^2}{n}\right)^n - \exp\left(x^2\right) \right| \le \exp\left(\alpha^2\right) \left| \exp\left( x\left(\phi \left(\frac{x^2}{n}\right) - 1\right)\right) -1\right| < \epsilon $$
...
Supongamos en primer lugar que $$\{f_n\}$$ es creciente. Nótese que en tal caso $$f(x) \ge f_n(x)$$ para todo $$x$$ y $$n$$. Sea $$\epsilon > 0$$:

**(1)** De la convergencia puntual se tiene que para todo $$x \in [a,b]$$ existe un natural $$n_x$$ tal que 

$$\forall n \ge n_x : \left| f_n(x) - f(x) \right| < \frac{\epsilon}{3}$$
...
$$\left| f_n(y) - f_n(x) \right| < \frac{\epsilon}{3}$$

$$\left| f(y) - f(x) \right| < \frac{\epsilon}{3}$$

**(3)** Utilizando la desigualdad triangular se obtiene que para todo $$y \in V_x$$ se cumple:

$$\left| f(y) - f_{n_x}(y) \right| \le \left| f(y) - f(x) \right| + \left| f(x) - f_{n_x}(x) \right| + \left| f_{n_x}(x) - f_{n_x}(y) \right| < \frac{\epsilon}{3} + \frac{\epsilon}{3} + \frac{\epsilon}{3} < \epsilon$$
...
$$\vert f(y) - f_n(y) \vert = f(y) - f_n(y) \le f(y) - f_{n_0}(y) \le f(y) - f_{n_{x_i}}(y) < \epsilon$$
...
**Ejemplo 2:**
: Estudia la convergencia puntual y uniforme de la sucesión de funciones $$\{f_n\}$$ donde $$f_n : [0,1] \rightarrow \mathbb{R}$$ esta definida por:
: $$f_n(x) = \left(1+\frac{x^2}{n}\right)^n \ \forall x \in \mathbb{R}$$ 
...
[^medias]: [La desigualdad de las medias. Mario Román](http://www.ugr.es/~fjperez/textos/sucesiones_series_funciones.pdf)
